...
class direita_def(SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and x[0]>delta-0.001
...
class InflowOutflow(UserExpression):
    ...
    def eval_cell(self, values, x, ufc_cell):
        ...
        values[1] = 0.0
        t = 1. - x[1]
        '''if (near(x[0], 0) or near(x[0], delta)) and 0.8<=x[1]<=1:
                values[0] = abs(self.gbar*( (1 - t/self.length)*(1+t/self.length) ) )#Entrada do Canal'''
        if near(x[0], 0) and 1-gap/2 <= x[1] <= 1+gap/2:
            # values[0] = abs(self.gbar*( (1 - t/self.length)*(1+t/self.length) ) )#Entrada do Canal
            values[0] = 102#self.gbar*2
...
class PP:
    ...
    def mesh_fun(self, mesh_file=None):
        ...
        if mesh_file is None and (self.forward_problem == '2D' or self.forward_problem == '2DSwirl'):
            ...
            else: mesh = RectangleMesh.create([Point(0.0,0.0),Point(delta,self.altura)], [int(self.N*delta), int(self.N*self.altura)], CellType.Type.quadrilateral)
    ...
    def subdominios(self):
        ...
        CompiledSubDomain("x[1] < " + str(1.-gap) ).mark(self.sub_domains, 2)
    ...
    def boundaries_cond(self):
        ...
        boundary = [
                ...
                DirichletBC(self.W.sub(0), self.G, "on_boundary && !(x[0] > "+str(delta-0.001)+")"),
                DirichletBC(self.W.sub(1), 0, "on_boundary && x[0] > " + str(delta-0.001)),
                ]
